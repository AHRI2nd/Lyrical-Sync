...
export function ModeSelectButton() {
  ...
  const currentLabel = spotifyMode
    ? isReady && trackName
      ? trackName.length > 14 ? trackName.slice(0, 12) + "…" : trackName
      : "Spotify"
    : t.modeFile;

  const currentColor = spotifyMode
    ? isLoggedIn && isReady
  ...
  const selectFile = () => {
    ...
    setSpotifyMode(false); setOpen(false);
  };
  const selectSpotify = () => {
    audioControls.pause();
    setSpotifyMode(true); setOpen(false);
  };
